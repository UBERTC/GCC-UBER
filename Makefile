...
BUILD_CONFIGARGS =  --cache-file=./config.cache '--enable-languages=c,c++,fortran,java,lto,objc' --program-transform-name='s,y,y,' --disable-option-checking --with-build-subdir="$(BUILD_SUBDIR)"
...
TARGET_CONFIGARGS = --cache-file=./config.cache --enable-multilib   '--enable-languages=c,c++,fortran,java,lto,objc' --program-transform-name='s,y,y,' --disable-option-checking --with-target-subdir="$(TARGET_SUBDIR)"
# This is the list of variables to export in the environment when
...
GCJ_FOR_TARGET=$(STAGE_CC_WRAPPER) $$r/$(HOST_SUBDIR)/gcc/gcj -B$$r/$(HOST_SUBDIR)/gcc/
